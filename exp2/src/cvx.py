...
if __name__ == '__main__':
    ...
    log_df = pd.DataFrame()
    output_dir = '../outputs'
    param_dir = os.path.join(output_dir, 'params_cvx')
    ...
    for rews, acts in tqdm(zip(rewards, actions), total=len(rewards)):
        ...
        tights.append(1 if np.sum(tts) == len(tts) else 0)
    ...
    log_df['ll'] = lls
    log_df['tight'] = tights
    log_df.to_csv(os.path.join(output_dir, 'log_cvx.csv'), index=False)
